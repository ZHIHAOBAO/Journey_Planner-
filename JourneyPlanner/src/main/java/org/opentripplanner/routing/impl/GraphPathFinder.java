...
public class GraphPathFinder {
    ...
    public static GraphPath joinPaths(List<GraphPath> paths) {
	...
	public static GraphPath joinPathsWithModes(List<GraphPath> paths, ArrayList<RoutingRequest> requestArray) {
		State lastState = paths.get(0).states.getLast();
		
		
		RoutingRequest options = requestArray.get(0);		
		lastState.stateData.setOpt(options);
		
		if (options.modes.getCar())
			lastState.stateData.nonTransitMode = TraverseMode.CAR;
        else if (options.modes.getWalk())
        	lastState.stateData.nonTransitMode = TraverseMode.WALK;
        else if (options.modes.getBicycle())
        	lastState.stateData.nonTransitMode = TraverseMode.BICYCLE;
        else
        	lastState.stateData.nonTransitMode = null;
		
		
        if (options.parkAndRide || options.kissAndRide) {
        	lastState.stateData.carParked = options.arriveBy;
        	lastState.stateData.nonTransitMode = lastState.stateData.carParked ? TraverseMode.WALK : TraverseMode.CAR;
        } else if (options.bikeParkAndRide) {
        	lastState.stateData.bikeParked = options.arriveBy;
        	lastState.stateData.nonTransitMode = lastState.stateData.bikeParked ? TraverseMode.WALK
        ...
        int requestCount = 1; // Start at the second index. The first request is correct
        
        for (GraphPath path : paths.subList(1, paths.size())) {
            lastState = newPath.states.getLast();
            
         // Set request in state here. Else it gets it from the last node, has the wrong mode data
            options = requestArray.get(requestCount);
            //++requestCount;
