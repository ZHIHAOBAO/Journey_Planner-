...
otp.widgets.tripoptions.TripOptionsWidget =
    ...
    initScrollPanel : function() {
        this.scrollPanel = $('<div id="'+this.id+'-scollPanel" class="notDraggable"></div>').appendTo(this.$());
        this.$().resizable({
            minHeight: 80,
...
otp.widgets.tripoptions.MyFavorite =
    otp.Class(otp.widgets.tripoptions.TripOptionsWidgetControl, {
	id : null,
    initialize: function (tripWidget) {
        console.log("init MyFavorite");
        
        bootbox.setLocale("en");
        
        otp.widgets.tripoptions.TripOptionsWidgetControl.prototype.initialize.apply(this, arguments);
        this.id = tripWidget.id + "-myFavorite";

        ich['otp-tripOptions-myFavorite']({
            widgetId : this.id
        }).appendTo(this.$());
        
        var this_ = this;
        
        this.tripWidget.module.on("favouriteSavePressed", $.proxy(function(pointNames, pointLatLngs) {
        	if(pointLatLngs == 0) {
                this_.favouritesError('No points entered');
                return;
            }
        	this_.showFavouriteSelectDialog(pointNames, pointLatLngs);
        }, this));
        
    },

    doAfterLayout : function() {
        this.loadFavouriteDropDown();
    },
    
    
    
    
    favouritesError: function (message) {
        bootbox.alert({
            title: 'Error',
            size: 'small',
            message: message
    ...
    loadFavourites: function () {
    	...
    	loadedFavourites = DefaultFavourites;
 
    	// Look at local storage to see if favourites are saved
        try {
            var localFavourites = localStorage.getItem("favourites");
        } catch (e) {
            this.favouritesError('Your web browser does not support storing settings locally.')
        }
        
        // If favourites saved.
        if (localFavourites) {
            var parseFavourites = JSON.parse(localFavourites);
            
            
            // Test they have valid information saved
            var validData = true;
	        
            for (var prop in parseFavourites) {
            	var test = parseFavourites[prop];
            	if((test.pointNames == undefined) || (test.pointLatLngs == undefined)) {
            		validData = false;
            		break;
            	}      	
            }
            ...
            if(validData) {
            	loadedFavourites = parseFavourites;
            }
        }
    ...
    loadFavouriteDropDown: function () {
    	var this_ = this;    	
        var favourites = this.loadFavourites();
        var menu = $('#favourite-dropdown').find('.dropdown-menu');
        menu.empty();
        $.each(favourites, function (key, _) {
            menu.append('<li><a>'+key+'</a></li>');
            menu.find('a:last').on('click', function () {
                this_.loadFavouriteFromDropdown(key);
            });              
        });
        // Add the edit option
        menu.append('<hr><li><a><b>Edit</b></a></li>');
        menu.find('a:last').on('click', function () {
        	this_.showEditFavouritesDialog();
        });    
    },

    showFavouriteSelectDialog: function (pointNames, pointLatLngs) {
    	var this_ = this;
    	
        var selectDialog = $('#favourite-select-dialog');
        selectDialog.find('.modal-title').text('Save to my favourites');

        // Clear body
        selectDialog.find('#favourite-save-point-list').empty();
        
        // Display the route points in list form
        for(var pointIdx = 0; pointIdx < pointLatLngs.length; ++pointIdx) {
        	...
        	var pointData = "Empty Point";
        
        	if(pointName != null && pointName != undefined && pointName != "") {
    			pointData = pointName;
    		}
        	else if(pointLatLng != null && pointLatLng != undefined && pointLatLng != "") {
        		pointData = "(" + pointLatLng.lat + ", " + pointLatLng.lng + ")";
    		}
    		
        	
        	selectDialog.find('#favourite-save-point-list').append("<li>"+pointData+"</li>");
        }
            
        selectDialog.find('#favourite-save-existingSelect').empty();
        
        // Fill the autocomplete with existing favourites for the name input                       
        $.each(this.loadFavourites(), function (favourite, _) {
        	selectDialog.find('#favourite-save-existingSelect').append([
        		'<option value="'+favourite+'">'
        	].join(''));           	
        });
        
        // Handle submit click
        selectDialog.find('#favourite-save-submit').off("click").on('click', function () {
        	// Retrieve Name used in input
        	var nameInputed = selectDialog.find('#favourite-save-nameInput')[0].value;      	
        	this_.saveAddressToFavourite(selectDialog, pointNames, pointLatLngs, nameInputed);
        });
        
        selectDialog.find('#favourite-save-nameInput')[0].value = '';
        ...
        $(".modal-open")[0].style = "padding-right: 0px;";
    },

    saveAddressToFavourite: function (dialog, pointNames, pointLatLngs, favouriteName) {
    	var this_ = this;
    	
        if (!favouriteName) {
            this.favouritesError('Favourite name not entered');
            return;
        }
        
        var favourites = this.loadFavourites();            
        var favouriteInfo = favourites[favouriteName];

        // If favourite doesn't exist, then create and add route
        if(favouriteInfo == undefined || favouriteInfo == null) {
        	favourites[favouriteName] = { addressList : [] };       
        	favouriteInfo = favourites[favouriteName];
   	
        	favouriteInfo.pointNames = pointNames;
        	favouriteInfo.pointLatLngs = pointLatLngs;
            this.saveFavourites(favourites);
            dialog.modal('hide');
            this.loadFavouriteDropDown();
        }
        // else ask if the user want to overwrite the favourite
        else {
            bootbox.confirm({
                title: 'confirm',
                message: 'Overwrite previous saved route named "'+favouriteName+'"?',
                callback: function (value) {
                    if (value) {
                    	favouriteInfo.pointNames = pointNames;
                    	favouriteInfo.pointLatLngs = pointLatLngs;
                        this_.saveFavourites(favourites);
                        dialog.modal('hide');
                        this_.loadFavouriteDropDown();
                    }
                }
            });
        }
    ...
    loadFavouriteFromDropdown : function (favourite) {
    	if (!favourite) {
    		this.favouritesError('Favourite is missing.');
    		return;
    	}
		...
		if (!(favourite in favourites)) {
			this.favouritesError('Favourite is unknown.');
			return;
		}
			 
		var favouriteInfo = favourites[favourite];
		var pointNames = favouriteInfo.pointNames;
		var pointLatLngs = favouriteInfo.pointLatLngs;

		this.tripWidget.module.favouriteLoadPoints(pointNames, pointLatLngs);
    },
    
    showEditFavouritesDialog : function() {       
    	var this_ = this;
    	var favourites = this.loadFavourites();
	
    	// Add favourites to the select option
    	$("#favourite-edit-select").empty();
        $.each(favourites, function (favouriteName, favourite) {
        	var name = favouriteName;            	
        	$("#favourite-edit-select").append('<option value="'+favouriteName+'">'+favouriteName+'</li>');
        });

        var selectedFavourite = $('#favourite-edit-select')[0].value;  	
    	var pointLatLngs = favourites[selectedFavourite].pointLatLngs;
    	var pointNames = favourites[selectedFavourite].pointNames;
    	
    	// Create new list of points part of this favourite
    	$("#favourite-edit-pointlist").empty(); 
    	for(var pointIdx = 0; pointIdx < pointLatLngs.length; ++pointIdx) {
    		var pointLatLng = pointLatLngs[pointIdx];
    		var pointName = pointNames[pointIdx];
    		...
    		$("#favourite-edit-pointlist").append('<li style="padding-left: 10px;">'+pointData+'</li>');
    	}
        
        // Set default select option
        $('#delete-favourite-button').attr('disabled', false);
                   
        // When the favourite select changes, display the points saved
        $('#favourite-edit-select').off('click').on('click', function() {
        	var selectedFavourite = $('#favourite-edit-select')[0].value;  	
        	var pointLatLngs = favourites[selectedFavourite].pointLatLngs;
        	var pointNames = favourites[selectedFavourite].pointNames;
        ...
        $('#delete-favourite-button').off('click').on('click', function () {
        	var selectedFavourite = $('#favourite-edit-select')[0].value;
            bootbox.confirm({
                title: 'confirm',
                size: 'small',
                message: 'Delete ' + selectedFavourite + '?',
                callback: function (value) {
                    if (value) {
                    	delete favourites[selectedFavourite];
                    	this_.saveFavourites(favourites);
                        $('#favourite-address-dialog').modal('hide');                            
                        this_.loadFavouriteDropDown();
                    }
                }
            })
        ...
        $('#favourite-address-dialog').modal({
            keyboard: false,
            backdrop: false,
        });
...
otp.widgets.tripoptions.LocationsSelector =
    ...
    initialize : function(tripWidget, geocoders) {
        ...
        this.tripWidget.module.on("loadPointsFromArray", $.proxy(function(pointNames, pointLatLngs) {
        	var pointsLength = pointLatLngs.length;
        	
        	// Clear and rest to default UI
	    	$("#"+this.id+"-locations").empty();
	    	this.uiPointCount = 0;
        	
        	while(this.uiPointCount < pointsLength || this.uiPointCount < 2) {
        		this.addNewPoint(); // Add new points until same as pointArray length
        	}
        	
        	// Fill in points
        	for(var pointIdx = 0; pointIdx < pointsLength; ++pointIdx) {
        		var pointName = pointNames[pointIdx];
        		var pointLatLng = pointLatLngs[pointIdx];
        		
        		var pointId = this.id +'-point' + pointIdx;
        		
        		var pointData = "(" + pointLatLng.lat + ", " + pointLatLng.lng + ")";
        		
        		if(pointName != null && pointName != undefined && pointName != ""){
        			pointData = pointName;
        		}
        		
        		$("#"+pointId).val(pointData);
        	}
        }, this));

        this.tripWidget.module.on("pointChanged", $.proxy(function(latlng, name, pointNum) {
        	// If we create flag and ui doesn't exist, then create
        	if(pointNum >= this.uiPointCount) {
        		this.addNewPoint();
        	}

        	var pointId = this.id +'-point' + pointNum;
            $("#"+pointId).val(name || '(' + latlng.lat.toFixed(5) + ', ' + latlng.lng.toFixed(5) + ')');
        }, this));
    },
    
    removePoint : function() {
    	if(this.uiPointCount <= 2) {
    		return; // Must always have two points
    	}
    	
    	var locations = $("#"+this.id+"-locations").children();
  	
    	var pointsLength = this.uiPointCount;	
    	locations[pointsLength-1].remove();
    
    	--this.uiPointCount;

    	var startLabelHeight = 3.8 * (this.uiPointCount-1);
    	$('#'+this.id+'-startLabel')[0].style.height = startLabelHeight+'em';
    	var test = $('#'+this.id+'-startLabel')[0].style;
    	
    	this.tripWidget.module.removeLastPoint();
    },

    addNewPoint : function() {
    	var this_ = this;
	   	   	
    	if(this.uiPointCount < this.tripWidget.module.maxPoints)
    	{  	  		
    		var modes = otp.config.modes;
	    	var pointId = this.id +'-point' + this.uiPointCount;
	    	
    		var addSelectMode = (this.uiPointCount > 0);
    		
    		// Height of the new html element container. Double if using mode select	
	    	var absoluteHeight = 2 * this.uiPointCount + "em";
 	
	    	if(this.uiPointCount > 1) {
	    		absoluteHeight = ((3.6 * this.uiPointCount) - 1.6) + "em";	
	    	}
	    	
	    	var inputAbsHeight = "0.0em";
	    	if(addSelectMode){
	    		inputAbsHeight = "1.85em";
	    	}
	    	
	    	var locationHtml = '<div style="position: absolute; top: '+absoluteHeight+'; left:0px; right: 30px;">';
	    	
	    	// Modes
	    	// TODO: Style so its inbetween points
	    	if(addSelectMode) {
	            locationHtml +=        '<div style="position: absolute; left:0px; right: 16px; top:0em; bottom: 0px;">' + _tr("Travel by") + ": ";
	            locationHtml += 			'<select id="'+pointId+'-modeSelect">';
		        _.each(modes, function(text, key) {
		        	locationHtml += 			'<option value="'+text+'">'+text+'</option>';
		        });
		        locationHtml 			+= '</select>';
	            locationHtml +=        '</div>';              	        
	    	}
	    	
            locationHtml +=        '<div style="position: absolute; left:0px; right: 16px; top:'+inputAbsHeight+'; bottom: 0px;">';
            locationHtml +=             '<input id="'+pointId+'" class="borderBox otp-tripOptions-loc-input">';
            locationHtml +=        '</div>';
            locationHtml +=        '<div style="position: absolute; top:'+inputAbsHeight+'; bottom: 0px; right: 0px; width: 16px; height: 19px;">';
            locationHtml +=             '<button id="'+pointId+'-dropdown" class="borderBox otp-tripOptions-loc-dropdownBtn">&#x25BC;</button>';
            locationHtml +=        '</div>';          
		    locationHtml +=   '</div>';
	   	
		    
		    $("#"+this.id+"-locations").append(locationHtml);
		    if(addSelectMode) {
		    	var pointNum = this.uiPointCount;
		    	
		    	// Set the mode initially
		    	var defaultMode = this_.tripWidget.module.mode;
		    	this_.tripWidget.module.setIntermediateMode(defaultMode, pointNum);
		    	 	
			    $('#'+pointId+'-modeSelect').change(function() {
			    	var modeValue = $('#'+pointId+'-modeSelect option:selected').text();  	
			    	var modeKey = (_.invert(modes))[modeValue];
			    	this_.tripWidget.module.setIntermediateMode(modeKey, pointNum);
	            }); 
		    }

	    	this.initInput($("#" + pointId), this.tripWidget.module.setPoint, this.uiPointCount);

	    	$("#"+pointId+'-dropdown').click($.proxy(function() {
	            $("#" + pointId).autocomplete("widget").show();
	        }, this));

	    	// Quick and dumb way to move end down with the new inputs. Reimplement if needed
	    	var startLabelHeight = 3.6 * this.uiPointCount;
	    	$('#'+this.id+'-startLabel')[0].style.height = startLabelHeight+'em';
    ...
    doAfterLayout : function() {
        ...
        this.addNewPoint(); //startpoint
        this.addNewPoint(); //endpoint

        $("#"+this.id+"-tianjia").click($.proxy(function(){
        	event.preventDefault();
        	otp.widgets.tripoptions.SortBySelector.call(thisSort, this.tripWidget, true);
        	this.addNewPoint();     	
        }, this));

        $("#"+this.id+"-delete").click($.proxy(function(){        	
        	this.removePoint();  	
        }, this));
        ...
        $("#"+this.id+"-resetPoints").click($.proxy(function() {
        	var module = this_.tripWidget.module;
            module.clearTrip();
            
            $("#"+this_.id+"-locations").empty();
	    	this_.uiPointCount = 0;
        	
        	while(this_.uiPointCount < 2) {
        		this_.addNewPoint(); // Add new points until same as pointArray length
        	}
        }, this));
        
        $('#myfavourite-savebutton').click(function () {
        	// Get all the locations entered and display. Then save one of the displayed
        	var savePointList = [];
        	var savedPoints = 0;
	
        	for(var pointIdx = 0; pointIdx < this_.uiPointCount; ++pointIdx)
    		{			 	 		
        		var pointText = $("#"+this_.id +'-point'+pointIdx)[0].value;  		
        		
        		if(!(pointText === null || pointText === "")){
        			savePointList[pointIdx] = pointText;            		
        		}
    		}
        	
            this_.tripWidget.module.favouriteSavePressed(savePointList);
        });
    ...
    initInput : function(input, setterFunction, pointNum) {
        var this_ = this;
        input.autocomplete({
            ...
            select: function(event, ui) {
                ...
                setterFunction.call(this_.tripWidget.module, latlng, false, result.description, pointNum);
                this_.tripWidget.inputChanged();
            }
        })
        .dblclick(function() {
...
otp.widgets.tripoptions.ModeSelector =
    ...
    initialize : function(tripWidget) {
        ...
        html += '</select>';
        html += '<div style="display: inline-block;" id="'+tripWidget.id+'-sortBySelector"></div>';
...
otp.widgets.tripoptions.SortBySelector =
  otp.Class(otp.widgets.tripoptions.TripOptionsWidgetControl, {

  id           :  null,

  sortBy        : otp.config.sortBy,

  optionLookup : null,
  modeControls : null,

  initialize : function(tripWidget, removeTransitOption) {
      console.log("init sort");
      thisSort = this;
      otp.widgets.tripoptions.TripOptionsWidgetControl.prototype.initialize.apply(this, arguments);

      this.id = tripWidget.id+"-sortBy";
      var html = "<div class='notDraggable'>Sort By: ";

      html += '<select id="'+this.id+'">';
      html += '<option>Please Select</option>'

    	  _.each(this.sortBy, function(text, key) {
        	  if(!removeTransitOption || key !== "MAX_2_TRANSITS") {
                  html += '<option value="'+key+'">'+text+'</option>';    		  
        	  }
          });
          html += '</select>';
          $("#" + tripWidget.id + "-sortBySelector").html(html);
  ...
  isApplicableForMode : function(mode) {
      return otp.util.Itin.includesSortBy(mode);
  }
});
