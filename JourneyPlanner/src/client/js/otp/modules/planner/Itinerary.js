...
otp.modules.planner.Itinerary = otp.Class({
    ...
    getBoundsArray : function() {	
        ...
        var minLat = start.lat;
        var minLng = start.lon;
        
        var maxLat = start.lat;
        var maxLng = start.lon;
        
        for(var legIdx = 0; legIdx < this.itinData.legs.length; ++legIdx) {
        	var point = this.itinData.legs[legIdx].to;
        	minLat = Math.min(point.lat, minLat);
        	minLng = Math.min(point.lon, minLng);
        	
        	maxLat = Math.max(point.lat, maxLat);
        	maxLng = Math.max(point.lon, maxLng);
        }
