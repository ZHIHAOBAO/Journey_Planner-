...
otp.widgets.ItinerariesWidget =
    ...
    updateItineraries : function(itineraries, queryParams, itinIndex) {
        ...
        for(var i=0; i<this.itineraries.length; i++) {
            ...
            .click(function(evt) {
                ...
                this_.module.clear();
                for(var j=0; j<this_.itineraries.length; j++) {
                	if(this_.itineraries[j] === itin) {
                        this_.module.drawItinerary(itin, false);                		
                	}else{
                        this_.module.drawItinerary(this_.itineraries[j], true);                		
                	}
                }
    ...
    renderHeaderContent : function(itin, index, parentDiv) {
        parentDiv.empty();
        var div = $('<div style="position: relative; height: 20px;margin-right: 150px;"></div>').appendTo(parentDiv);
        ...
        var widthPx = pxSpan * (itinSpan / maxSpan) + 30;
	...
	/*timeStr = timeStr.substring(0, timeStr.length - 1);*/
        ...
        div.append('<div style="position: absolute;right: -150px;"> '+Math.floor(itin.itinData.duration / 60)+' mins</span>');
